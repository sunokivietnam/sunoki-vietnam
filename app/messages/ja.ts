const ja = {
  language: {
    ja: "JP",
    vi: "VI",
  },

  nav: {
    company: "会社案内",
    service: "サービス",
    works: "施工事例",
    process: "施工の流れ",
    contact: "お問い合わせ",
  },

  buttons: {
    estimate: "無料相談",
    contact: "お問い合わせ",
    more: "詳しく見る",
  },

  hero: {
  brand: "KOKORO LIVING",

  title: `心を込めた、
空間づくり。`,

  titleMobile: `心を込めた、
空間づくり。`,

  description:
    "日本品質の設計・施工で、\nベトナムの暮らしに心地よさという価値を。\n住宅・店舗・水まわりまで、一つひとつ丁寧に創ります。",

  descriptionMobile:
    "日本品質の設計・施工で、\nベトナムの暮らしに\n心地よさという価値を。\n\n住宅・店舗・水まわりまで、\n一つひとつ丁寧に創ります。",

  works: "施工事例を見る",

  consultation: "無料相談する",

  scroll: "SCROLL",

  copyright: "Designed by SUNOKI VIETNAM",
},
  about: {
    philosophy: "PHILOSOPHY",

   title: "KOKORO Livingとは",

// PC
descriptionLead: "それぞれの空間には、それぞれの価値があります。",

description: `帰る場所となる住まい、
創造力を育むオフィス、
大切なお客様を迎えるレストラン。

それぞれの空間には物語があり、
思い出が積み重なっていきます。`,

description2: `KOKORO Livingでは、
美しい空間とはデザインだけではなく、
心地よさ・使いやすさ・人とのつながりが
生まれることだと考えています。

日本品質の設計・施工を通して、
細部まで丁寧にこだわり、
一人ひとりに寄り添う空間づくりを行っています。`,

// Mobile
descriptionLeadMobile: `空間には、
それぞれの価値がある。`,

descriptionMobile: `住まいも、
創造を生み出すオフィスも、
大切な人を迎えるレストランも、

すべての空間には
物語があり、

かけがえのない時間が
刻まれていきます。`,

description2Mobile: `KOKORO Livingは、

美しい空間とは、
デザインだけではなく、

心地よさ、
使いやすさ、
そして人と人とのつながりを
育む場所であると考えています。

日本品質の設計・施工で、
一つひとつの細部まで丁寧に仕上げ、
お客様らしさが息づく、
永く愛される空間を
お届けします。`,

    collection: "KOKORO COLLECTION",

   collectionTitle:
  "日本品質を彩る、こだわりの素材と意匠。",

collectionTitleMobile:
  "日本品質を彩る、\nこだわりの素材と意匠。",

    collectionDescription:
      "木・光・和の美しさを大切にし、細部まで心を込めた空間づくりを行っています。",
      collectionDescriptionMobile:
`木・光・和の美しさを大切にし、
細部まで心を込めた
空間づくりを行っています。`,

    woodTitle: "天然木材",

    woodDescription:
  "木の温もりが心地よい空間を演出します。",

    latticeTitle: "建具・格子",

    latticeDescription:
`日本建築ならではの
美しい意匠を取り入れます。`,

    lightingTitle: "照明器具",

    lightingDescription:
"やわらかな光で、上質で落ち着いた空間を演出します。",

lightingDescriptionMobile:
`やわらかな光で、
上質で落ち着いた空間を
演出します。`,

    bonsaiTitle: "盆栽・和小物",

    bonsaiDescription:
  "盆栽や和小物が、日本らしい趣と癒やしを空間に添えます。",

bonsaiDescriptionMobile:
  "盆栽や和小物で、\n和の趣と癒やしを添えます。",
  },

  service: {
    subtitle: "SERVICE",

    title: "サービス",

    description:
      "住宅から店舗・水まわり・内装まで、日本品質の設計・施工をご提供します。",

      descriptionMobile:
`住宅から店舗・水まわり・内装まで、
日本品質の設計・施工をご提供します。`,

    worksButton: "施工事例を見る",

    items: [
     {
  category: "RESIDENCE",

  title: "住宅内装",

  description:
    "新築・リフォーム・リノベーションまで、日本品質で快適な住まいをご提案します。",

  descriptionMobile:
`新築・リフォーム・リノベーションまで、
日本品質で快適な住まいをご提案します。`,
},      {
  category: "RESTAURANT",

  title: "店舗・オフィス",

  description:
    "店舗デザインからオフィス施工まで、集客と使い易さを両立した空間を実現します。",

  descriptionMobile:
`店舗デザインからオフィス施工まで、
集客と使い易さを両立した空間を実現します。`,
},
      {
  category: "BATHROOM",

  title: "水まわり",

  description:
    "キッチン・浴室・トイレなど、水まわりを美しく機能的にリフォームします。",

  descriptionMobile:
`キッチン・浴室・トイレなど、
水まわりを美しく機能的にリフォームします。`,
},
      {
  category: "ZEN SPACE",

  title: "庭園・和空間",

  description:
    "日本の美意識を感じる和空間をデザイン。\n光・素材・陰影を活かし、心が落ち着く上質な空間をご提案します。",

  descriptionMobile:
`日本の美意識を感じる和空間をデザイン。
光・素材・陰影を活かし、心が落ち着く上質な空間を
ご提案します。`,
},
    ],
  },

  works: {
    subtitle: "WORKS",

    title: "施工事例",

    description:
  "SUNOKI VIETNAMが手掛けた\n住宅・店舗の施工事例をご紹介します。",

    residence: {
      subtitle: "RESIDENCE",

      title: "住宅",

      description:
  "日本品質の住空間。\n快適性・機能性・美しさを兼ね備えた暮らしを実現します。",

descriptionMobile:
  "日本品質の住空間。\n快適性・機能性・美しさを兼ね備えた\n暮らしを実現します。",

      items: [
        {
          title: "寝室",
          subtitle: "BEDROOM",
        },
        {
          title: "キッチン",
          subtitle: "KITCHEN",
        },
        {
          title: "トイレ",
          subtitle: "TOILET",
        },
        {
          title: "バスルーム",
          subtitle: "BATHROOM",
        },        {
          title: "洗面台",
          subtitle: "WASHBASIN",
        },
        {
          title: "和モダン空間",
          subtitle: "JAPANESE MODERN",
        },
      ],
    },

    commercial: {
      subtitle: "COMMERCIAL",

      title: "店舗",

      description:
  "飲食店・オフィス・商業空間の設計施工。\nデザイン性と機能性を両立した空間をご提案します。",

descriptionMobile:
  "飲食店・オフィス・商業空間の設計施工。\nデザイン性と機能性を両立した空間を\nご提案します。",
      items: [
        {
          title: "GF客席",
          subtitle: "GROUND FLOOR",
        },
        {
          title: "縁側",
          subtitle: "ENGAWA",
        },
        {
          title: "ドリンクバー",
          subtitle: "DRINK BAR",
        },
        {
          title: "酒瓶ディスプレイ",
          subtitle: "SAKE DISPLAY",
        },
        {
          title: "かまくら席",
          subtitle: "KAMAKURA SEAT",
        },
        {
          title: "2F個室",
          subtitle: "PRIVATE ROOM",
        },
        {
          title: "寿司カウンター",
          subtitle: "SUSHI COUNTER",
        },
      ],
    },   model: {
  title: "3Dモデルルーム",

  description:
    "完成後の空間を3Dウォークスルーでご体感ください。",

  descriptionMobile:
`完成後の空間を
3Dウォークスルーでご体感ください。`,
},
  },
  why: {
    subtitle: "WHY SUNOKI VIETNAM",

    title: "日本品質が、\n選ばれる理由。",

    description:
      "細部へのこだわりと、\n日本品質の設計・施工で、\n安心して任せられる空間づくりを実現します。",

       descriptionMobile:
    "細部へのこだわりと、\n日本品質の設計・施工で、\n安心して任せられる\n空間づくりを実現します。",

   features: [
  {
  title: "日本品質",

  description:
    `品質管理から仕上げまで、
日本基準の丁寧な施工を行います。`,

  descriptionMobile:
`品質管理から仕上げまで、
日本基準の丁寧な施工を行います。`,
},
{
  title: "日本人一級建築士",

  description:
    `日本人一級建築士が設計・品質を監修。
    安心できる空間づくりをご提案します。`,

  descriptionMobile:
`日本人一級建築士が設計・品質を監修。
安心できる空間づくりをご提案します。`,
},
  {
  title: "日本で教育を受けたスタッフ",

  description:
    `日本で学び経験を積んだスタッフが、
    現場で品質を支えます。`,

  descriptionMobile:
`日本で学び経験を積んだスタッフが、
現場で品質を支えます。`,
},
  {
  title: "日本語・ベトナム語対応",

  description:
    `日本語・ベトナム語の両方で、スムーズに
    ご相談いただけます。`,

  descriptionMobile:
`日本語・ベトナム語の両方で、
スムーズにご相談いただけます。`,
},
  {
  title: "設計・施工ワンストップ",

  description:
    `設計から施工まで一貫対応。
    安心してお任せいただけます。`,

  descriptionMobile:
`設計から施工まで一貫対応。
安心してお任せいただけます。`,
},
  {
  title: "アフターサポート",

  description:
    `完成後も長く安心して暮らせるよう、
    継続してサポートいたします。`,

  descriptionMobile:
`完成後も長く安心して暮らせるよう、
継続してサポートいたします。`,
},
],
  },
  process: {
    subtitle: "PROCESS",

    title: "施工の流れ",

   description:
  "お問い合わせから施工完了まで、一つひとつ丁寧にサポートいたします。",

descriptionMobile:
`お問い合わせから施工完了まで、
一つひとつ丁寧にサポートいたします。`,

    steps: [
      {
  number: "01",

  title: "お問い合わせ",

  titleMobile: "お問い合わせ",

  description:
    `フォーム・電話・メールから
お気軽にご相談ください。`,

  descriptionMobile:
`フォーム・電話・メールから
お気軽にご相談ください。`,
},
      {
  number: "02",

  title: "ヒアリング・\n現地調査",

  titleMobile:
`ヒアリング・現地調査`,

  description:
   `ご要望やご予算を伺い、
現地調査を行います。`,

  descriptionMobile:
`ご要望やご予算を伺い、
現地調査を行います。`,
},
      {
  number: "03",

  title: "3Dデザイン・\nお見積り",

  titleMobile:
`3Dデザイン・お見積り`,

  description:
    `最適なプランとお見積りを
ご提案いたします。`,

  descriptionMobile:
`最適なプランとお見積りを
ご提案いたします。`,
},
      {
  number: "04",

  title: "管理・施工開始",

  titleMobile:
`管理・施工開始`,

  description:
    `日本品質の施工管理で丁寧に
    工事を進めます。`,

  descriptionMobile:
`日本品質の施工管理で
丁寧に工事を進めます。`,
},      {
  number: "05",

  title: "完成・お引渡し",

  titleMobile:
`完成・お引渡し`,

  description:
    `最終確認後、
    お客様へお引渡しいたします。`,

  descriptionMobile:
`最終確認後、
お客様へお引渡しいたします。`,
},
    ],
  },
   reviews: {
    subtitle: "TESTIMONIALS",

    title: "お客様の声",

    description:
      "実際に施工をご依頼いただいたお客様の声をご紹介します。",
descriptionMobile:
`実際に施工をご依頼いただいた
お客様の声をご紹介します。`,

    items: [
  {
    name: "Nguyen Van A",
    area: "District 3, HCMC",
    comment:
      "日本品質の施工で、細かい部分までとても丁寧でした。完成した家族全員が満足しています。",
  },
  {
    name: "Tran Thi B",
    area: "Thu Duc City",
    comment:
      "打ち合わせから完成まで安心して任せられました。予定通りに工事が終わり感謝しています。",
  },
  {
    name: "Yamada",
    area: "Binh Thanh",
    comment:
      "デザイン性と機能性の両方を兼ね備えた理想の空間になりました。またお願いしたいです。",
  },
]
   },
   modelPlans: {
  subtitle: "MODEL PLANS",

  title: "参考プラン",
  titleMobile: "参考プラン",

  description:
    "実際の設計プランをもとに、面積・仕様・概算金額の参考例をご紹介します。",

  descriptionMobile:
    "実際の設計プランをもとに、\n面積・仕様・概算金額の\n参考例をご紹介します。",

  area: "面積",

  unitPrice: "㎡単価",

  grade: "グレード",

  specification: "仕様・設備",

  estimatedCost: "概算金額",

  viewGallery: "施工写真を見る",
closeGallery: "施工写真を閉じる",

viewSpecification: "仕様を見る",
hideSpecification: "仕様を閉じる",
  referencePrice: "参考価格",
  from: "FROM",
  excludingVat: "税抜",

  condoA: {
    kitchen: [
      "シューズキャビネット",
      "冷蔵庫収納",
      "上下キャビネット",
      "人工大理石カウンター",
      "ガラスバックパネル",
      "GARIS収納システム",
      "レンジフード",
      "IHクッキングヒーター",
      "シンク・水栓",
    ],

    living: [
      "ソファ",
      "ダイニングテーブル・チェア",
      "テレビキャビネット",
    ],

    bathroom: [
      "洗面台キャビネット",
      "洗面ボウル",
      "水栓",
      "シャワーセット",
      "トイレ",
      "壁タイル",
      "ガラスシャワーパーティション",
    ],

    masterBedroom: [
      "ベッド",
      "ベッドサイドテーブル",
      "ワードローブ",
      "ドレッシングテーブル",
      "チェア",
      "装飾壁パネル",
    ],

    secondBedroom: [
      "ベッド",
      "ベッドサイドテーブル",
      "装飾壁パネル",
      "ワードローブ",
      "ドレッシングテーブル",
      "チェア",
    ],

    interiorFinish: [
      "床仕上げ",
      "幅木",
      "壁塗装",
      "電気配線",
      "LED間接照明",
      "LED電源・タッチセンサー",
      "ダウンライト",
      "室内ドア",
    ],
  },
  condoB: {
  kitchen: [
    "造作収納キャビネット",
    "下部キッチンキャビネット",
    "上部キッチンキャビネット",
    "冷蔵庫収納",
    "キッチンカウンター（ホワイトマーブル）",
    "6人用ダイニングテーブルセット",
    "INAXタイル壁面",
    "Malloca 3口IHクッキングヒーター",
    "Malloca シンク",
    "Malloca キッチン水栓",
    "Malloca レンジフード",
    "GARIS ナイフ・まな板・ボトル収納",
    "GARIS キッチン収納",
    "GARIS スパイスラック",
    "GARIS ダストボックス",
  ],

  kitchenPrice: "127,679,000 VNĐ",

  living: [
    "3人掛けソファ",
    "ソファテーブル",
    "テレビキャビネット",
    "テレビ壁面装飾",
    "テレビ壁面パネル",
    "テレビ壁面パネル（追加）",
    "テレビ横装飾キャビネット",
  ],

  livingPrice: "56,126,000 VNĐ",

  bathroom: [
    "洗面台下キャビネット",
    "ミラーキャビネット",
    "INAX 電子便座付きトイレ",
    "INAX 洗面ボウル",
    "INAX 洗面水栓",
    "INAX タイル壁面",
  ],

  bathroomPrice: "44,800,000 VNĐ",

  laundry: [
    "洗濯室キャビネット",
    "洗面台下キャビネット",
    "ミラーキャビネット",
    "INAX タイル壁面",
  ],

  laundryPrice: "31,860,000 VNĐ",

  masterBedroom: [
    "ベッド",
    "ベッドサイドテーブル",
    "ヘッドボード壁面",
    "ドレッシングテーブル",
    "ドレッシングチェア",
    "LEDミラー",
    "テレビキャビネット",
    "テレビ背面壁面",
    "ワードローブ",
    "ワードローブ補強材",
    "収納キャビネット",
  ],

  masterBedroomPrice: "76,456,000 VNĐ",

  smallBedroom: [
    "ベッド",
    "ベッドサイドテーブル",
    "ヘッドボード壁面",
    "デスク",
    "デスクチェア",
    "ワードローブ",
    "テレビキャビネット",
    "テレビ背面壁面",
  ],

  smallBedroomPrice: "41,260,000 VNĐ",

  interiorFinish: [
    "天然木フローリング（マンゴー材）",
    "幅木・見切り材",
    "日本式壁紙",
    "電気配線・移設",
    "LED間接照明",
    "LED電源・タッチセンサー",
    "ダウンライト（3色切替・Panasonic）",
    "室内ドア（MDF・3本ライン）",
  ],
  interiorFinishPrice: "81,200,000 VNĐ",
   },

villa: {
  kitchenDining: {
  title: "キッチン・ダイニング",
  items: [
    "ダイニングテーブル",
    "ダイニングチェア ×6",
    "キッチン床収納キャビネット",
    "人工大理石カウンター",
    "下部キッチンキャビネット",
    "上部キッチンキャビネット",
    "INAX キッチン壁タイル",
    "Malloca 3口IHクッキングヒーター",
    "Malloca シンク",
    "Malloca キッチン水栓",
    "Malloca レンジフード",
  ],
  price: "328,045,000 VNĐ",
},

  living: {
  title: "リビング",
  items: [
    "テレビキャビネット",
    "段差付きMDF造作",
    "MDF壁面",
    "追加MDF造作",
    "テレビ横装飾キャビネット",
    "ソファ背面壁面",
    "ソファ",
    "1人掛けソファチェア",
    "ソファテーブル",
    "シューズキャビネット",
  ],
  price: "223,310,000 VNĐ",
},

  livingVipEntertainment: {
  title: "VIPリビング・エンターテインメント",
  items: [
    "テレビキャビネット・装飾キャビネット",
    "造作キャビネット",
    "造作家具",
  ],
  price: "645,453,000 VNĐ",
},

 decorativeStorage: {
  title: "造作収納",
  items: [
    "天井見切り・MDF造作",
    "MDF造作収納",
    "MDF造作キャビネット",
  ],
  price: "61,680,000 VNĐ",
},

  masterBedroom: {
  title: "マスターベッドルーム",
  items: [
    "MDFメラミン化粧板のルーバー壁（ベッドヘッド）",
    "1.8mベッド",
    "ベッドサイドテーブル",
    "MDFメラミン化粧板の造作装飾キャビネット",
    "MDFメラミン化粧板のワードローブ",
  ],
  price: "219,156,000 VNĐ",
},

  smallBedroom: {
  title: "子供部屋",
  items: [
    "MDFメラミン化粧板のルーバー壁（ベッドヘッド）",
    "1.6mベッド ×2",
    "ベッドサイドテーブル ×2",
    "MDFメラミン化粧板の造作装飾キャビネット",
    "MDFメラミン化粧板のワードローブ",
  ],
  price: "177,724,000 VNĐ",
},

  bathroomWc: {
  title: "バスルーム・トイレ",
  items: [
    "洗面台吊りキャビネット",
    "ソリッドサーフェス天板",
    "ミラーキャビネット",
    "トイレドア",
    "ガラスシャワーパーティション",
    "INAX 電子便座付きトイレ",
    "INAX 洗面ボウル",
    "INAX 洗面水栓",
    "INAX 壁タイル",
    "INAX 据置型バスタブ 1.7m",
    "洗面台吊りキャビネット",
    "ソリッドサーフェス天板",
    "ミラーキャビネット",
    "トイレドア",
    "ガラスシャワーパーティション",
    "INAX 電子便座付きトイレ",
    "INAX 洗面ボウル",
    "INAX 洗面水栓",
  ],
  price: "256,972,000 VNĐ",
},

  laundry: {
  title: "ランドリー",
  items: [
    "ランドリーキャビネット",
    "洗面台吊りキャビネット",
    "ソリッドサーフェス天板",
    "洗面水栓",
    "各階室内ドア",
  ],
  price: "47.249.000 VNĐ",
},

 roughCompletionFinishing: {
  title: "仕上げ工事",
  items: [
    "天然木フローリング（マンゴー材）",
    "同色の幅木・見切り材",
    "エフェクト塗装",
    "電気配線・移設",
    "LEDテープライト（白色・電球色・昼白色）",
    "LED電源・タッチセンサー",
    "ダウンライト（3色切替・Panasonic）",
    "各階室内ドア ×5",
  ],
  price: "924.000.000 VNĐ",
},
},

  notice:
    "※掲載価格は参考価格です。設計内容・仕様・施工条件により変動します。",

  noticeMobile:
    "※掲載価格は参考価格です。\n設計内容・仕様・施工条件により\n変動します。",

  plans: [
    
    {
  label: "PLAN 01",
  name: "Condo A",

  image: "/images/modelplans/condo-a/main.jpg",

  gallery: [
    "/images/modelplans/condo-a/07.jpg",
    "/images/modelplans/condo-a/01.jpg",
    "/images/modelplans/condo-a/02.jpg",
    "/images/modelplans/condo-a/03.jpg",
    "/images/modelplans/condo-a/04.jpg",
    "/images/modelplans/condo-a/05.jpg",
    "/images/modelplans/condo-a/06.jpg",
  ],

  area: "68㎡",

  unitPrice: "約3,126,000 VNĐ /㎡",

  grade: "Standard",

  specifications: [
    "オーク材",
    "収納",
    "照明",
    "家具",
  ],

  estimatedCost: "212,554,500 VNĐ",
},
    {
  label: "PLAN 02",
  name: "Condo B",

  image: "/images/modelplans/condo-b/main.jpg",

  gallery: [
    "/images/modelplans/condo-b/01.jpg",
    "/images/modelplans/condo-b/02.jpg",
    "/images/modelplans/condo-b/03.jpg",
    "/images/modelplans/condo-b/04.jpg",
    "/images/modelplans/condo-b/05.jpg",
    "/images/modelplans/condo-b/06.jpg",
    "/images/modelplans/condo-b/07.jpg",
  ],

  area: "100㎡",

  unitPrice: "約8,594,000 VNĐ /㎡",

  grade: "Premium",

  specifications: [
    "ウォールナット",
    "キッチン",
    "照明",
    "家具",
  ],

  estimatedCost: "859.381.000 VNĐ",
},
    {
  label: "PLAN 03",
  name: "Villa Residence",

  image: "/images/modelplans/villa/main.jpg",

  gallery: [
    "/images/modelplans/villa/12.jpg",
    "/images/modelplans/villa/01.jpg",
    "/images/modelplans/villa/02.jpg",
    "/images/modelplans/villa/03.jpg",
    "/images/modelplans/villa/04.jpg",
    "/images/modelplans/villa/05.jpg",
    "/images/modelplans/villa/06.jpg",
    "/images/modelplans/villa/07.jpg",
    "/images/modelplans/villa/08.jpg",
    "/images/modelplans/villa/09.jpg",
    "/images/modelplans/villa/10.jpg",
    "/images/modelplans/villa/11.jpg",
  ],

  area: "425㎡",

  unitPrice: "約16,020,000 VNĐ /㎡",

  grade: "Luxury",

  specifications: [
    "天然木",
    "造作家具",
    "間接照明",
    "和モダン",
  ],

  estimatedCost: "2,883,589,000 VNĐ",
},
  ],
},
     group: {
    subtitle: "SUNOKI GROUP",

title:
  "1963年から受け継ぐ技術と品質をベトナムへ。",

titleMobile:
`1963年から受け継ぐ
技術と品質を
ベトナムへ。`,

description:
  "日本で60年以上培ってきた施工技術と品質管理を受け継ぎ、\nベトナムでも安心して任せられる空間づくりをご提供します。",

descriptionMobile:
`1963年創業、
60年以上にわたり培った
日本品質の施工・品質管理。

ベトナムで安心できる
住空間・商業空間を
提供しています。`,

    japan: {
  title: "サンオキ株式会社",
  since: "SINCE 1963",

  items: [
    "60年以上の技術と品質管理",
    "軽鉄・ボード工事専門",
    "日本全国で豊富な施工実績",
    "日本品質をベトナムへ継承",
  ],

  itemsMobile: [
    "60年以上の技術と品質管理",
    "軽鉄・ボード工事専門",
    "日本全国で豊富な施工実績",
    "日本品質をベトナムへ継承",
  ],
},
    vietnam: {
      title: "SUNOKI VIETNAM",
      since: "SINCE 2018",
      items: [
        "住宅・店舗・オフィス内装",
        "設計・施工をワンストップ対応",
        "日本品質の空間づくり",
        "ベトナムで安心の施工体制",
      ],
      itemsMobile: [
  "住宅・店舗・オフィス内装",
  "設計・施工をワンストップ対応",
  "日本品質の空間づくり",
  "ベトナムで安心の施工体制",
],
    },

    websiteTitle: "日本本社について",

websiteDescription:
"1963年創業。軽鉄・ボード工事を専門とし、日本全国で豊富な施工実績を積み重ねてきたサンオキ株式会社の詳細をご覧いただけます。",

websiteDescriptionMobile:
`1963年創業。
軽鉄・ボード工事を専門とする
サンオキ株式会社について
詳しくご覧いただけます。`,

    button: "日本本社サイトを見る",
  },
  contact: {
    subtitle: "CONTACT",

    title: "お問い合わせ",

    description:
      "住まい・店舗づくりのご相談、お見積りはお気軽にお問い合わせください。",

    company: "会社名",

    name: "お名前",

    email: "メールアドレス",

    phone: "電話番号",

    message: "お問い合わせ内容",

    submit: "送信する",

    thanks: "お問い合わせありがとうございます。",
  },

  footer: {
    menu: "MENU",

    title: "日本品質を、\nベトナムへ。",

titleMobile: "日本品質を、ベトナムへ。",

    description:
      `住宅・店舗・水まわり・内装まで。
日本品質の設計・施工で、
ベトナムに安心と快適な空間をお届けします。`,
      descriptionMobile: `住宅・店舗・水まわり・内装まで。
日本品質の設計・施工で、ベトナムに
安心と快適な空間をお届けします。`,
  },
};

export default ja;